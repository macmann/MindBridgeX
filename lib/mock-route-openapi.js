...
function buildRequestBody(route) {
  if (!route || !route.requestSampleBody) {
    return null;
  }

  const parsed = parseJson(route.requestSampleBody);
  const hasSchema = route.requestSchema && typeof route.requestSchema === 'object';
  ...
  if (parsed !== null) {
    ...
    content['application/json'].example = route.requestSampleBody;
  }
